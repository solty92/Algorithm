...
public class Q7562 {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < testCase; i++) {
			...
			isVisited = new boolean[edge][edge];
			...
			bfs();
			bw.write(chessboard[destination[0]][destination[1]] + "\n");
		}
		...
		while(!q.isEmpty()) {
			...
			for (int i = 0; i < 8; i++) {
				...
				if(rowChildPoint >= 0 && columnChildPoint >= 0 && rowChildPoint < edge && columnChildPoint < edge && !isVisited[rowChildPoint][columnChildPoint]) {
					...
					chessboard[rowChildPoint][columnChildPoint] = chessboard[motherPoint[0]][motherPoint[1]] + 1;
					q.offer(new int[] {rowChildPoint, columnChildPoint});
					
					if(rowChildPoint == destination[0] && columnChildPoint == destination[1]) return;
